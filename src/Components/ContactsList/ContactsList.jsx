      ...
      <table className={`${styles.table}`}>
        ...
        <tbody>
          {filteredContacts.length ? (
            filteredContacts.map((item) => <ContactRow setIsEditing={setIsEditing} deleteHandler={deleteHandler} setSelectedContact={setSelectedContact} selectedContact={selectedContact} setContact={setContact}  key={item.id} contact={item}  />)
          ) : (
            <tr>
              <td colSpan={6}>
